...
class CreateProductVariantUseCaseTest {
  ...
  void should_create_variant_successfully() {
    ...
    CreateProductVariantRequest request = new CreateProductVariantRequest(
        "SKU-NEW", "123456789012", new BigDecimal("10.0"), new BigDecimal("15.0"), 50, Map.of("color", "red")
    );

    Property property = new Property(PropertyId.from(1), "color");

    when(productRepository.findById(productId)).thenReturn(Optional.of(product));
    when(productRepository.existsBySku(Sku.from("SKU-NEW"))).thenReturn(false);
    when(propertyRepository.findByName("color")).thenReturn(Optional.of(property));
    ...
    ProductResponse response = createProductVariantUseCase.execute(productId.value().toString(), request);
